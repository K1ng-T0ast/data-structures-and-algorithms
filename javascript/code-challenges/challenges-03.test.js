...
const typeNum = (arr) => {
  let onlyNumbers = arr.filter(n => typeof n === 'number');
  return onlyNumbers;
};
...
const containsAnd = (arr) => {
  let onlyStrings = arr.filter(s => typeof s === 'string' && s.includes('and'));
  return onlyStrings;
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let filterStat = arr.filter(stat => stat.baseStat > minBaseStat);
  return filterStat;
};
...
const getStatName = (arr, minBaseStat) => {
  let filterStat = arr.filter(stat => stat.baseStat > minBaseStat);
  let statName = filterStat.map(stat => stat.stat.name);
  return statName;
};
...
const getCharactersWithoutChildren = (arr) => {
  let parentsWithoutChildren = arr.filter(character => !character.children);
...
const evenOddNumericValues = (arr) => {
  let numericValues = arr.filter(value => typeof value === 'number');
  let evenOddLabels = numericValues.map(value => value % 2 === 0 ? 'even' : 'odd');
  return evenOddLabels;
};
