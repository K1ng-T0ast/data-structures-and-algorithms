...
const count = (target, input) => {
  const totalCount = input.reduce((acc, currentValue) => {
    const currentCount = currentValue.reduce((acc2, currentValue2) => {
      return currentValue2 === target ? acc2 + 1 : acc2;
    }, 0);
...
const totalSum = (input) => {
  const sum = input.reduce((accumulator, currentArray) => {
    const currentSum = currentArray.reduce((accumulator2, newValue) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  const result = input.map((curArr) => {
    const filteredArr = curArr.filter((val) => typeof val === 'number' && val % 5 === 0);
    const newArr = filteredArr.map((val)=> Math.pow(2, val));
    return newArr;
  });
  return result;
};
...
let findMaleAndFemale = (data) => {
  const maleAndFemale = data.filter(
    (character) => character.gender === 'male' || character.gender === 'female'
  );
  const name = maleAndFemale.map((character)=> character.name);
  const result = name.join(' and ');
  return result;
};
...
let findShortest = (data) => {
  const shortCharacter = data.reduce((findShortest, currentCharacter) => {
    return parseInt(currentCharacter.height) < parseInt(findShortest.height) ? currentCharacter : findShortest;
  });
